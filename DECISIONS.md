...
1. **완료된 작업**:
   ...
   - **지식 미등록 지역/질문 자동 큐 수집 파이프라인 연동 (`App.jsx`, `voraQnaMatcher.js`)**:
     - 코스 생성이든 대화든, 공식 지식베이스에 아직 미등록된 도시나 미학습 질문이 인입될 경우 **자동으로 관리자 미답변 큐(`vora_unanswered_qna`)에 `[신규 지역 지식 필요: 도시명]` 형태로 100% 실시간 자동 적재**되도록 자가 학습 플라이휠 연동 완성!
2. **다음 작업 1순위**:
