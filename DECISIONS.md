...
1. **완료된 작업**:
   ...
   - **복합 지명(제주·서귀포, 통영·거제 등) 자동 분해 & 실시간 권역 전수 조회 파이프라인 완비 (`tourApi.js` & `localItineraryGenerator.js`)**: '제주·서귀포' 등 복합 도시명 입력 시 단일 시군구로 묶이지 않고 권역 전체의 공공데이터를 전수 수신하도록 정규화 분해 로직 장착.
2. **다음 작업 1순위**:
