...
- **🎨 30초 퀵 투어(InteractiveQuickTour) 한반도 SVG 지도 & 지리적 위치 핀 애니메이션 & 1일 대표 연계 코스 장착**:
  - Step 1: 실제 한반도 지형 실루엣(SVG) + 도시 클릭 시 서울(북서), 강릉(동북), 부산(동남), 제주(남쪽 섬)로 핀이 부드럽게 지리적 이동하는 펄스 애니메이션 탑재.
  - Step 1: 도시 클릭 시 사진 하단에 `[ ✨ 1일 대표 코스: 1 ➔ 2 ➔ 3 ]` 연계 동선 칩을 4개국어(한·영·일·중)로 실시간 렌더링.
