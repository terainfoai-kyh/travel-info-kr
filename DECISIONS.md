...
### [2026-09-02 (수)]
1. **완료된 작업**:
   - **지도 클릭 시 우측 4K 매거진 프리뷰 화면의 기본 사진 순간 플리커(Flicker) 100% 완전 해결 (`DesktopMapExplorer.jsx`)**:
     - **원인 분석**: 지도 위 임의의 위치(포항, 안동, 여수 등 226개 시·군)를 클릭했을 때, TourAPI 비동기 사진을 불러오는 약 0.2초 동안 기본 이미지(`theme-gyeongbokgung.jpg`)가 먼저 찰나로 렌더링되었다가 실제 사진으로 교체되는 플리커(Flicker) 발생.
     - **해결 및 100% 원본 선명도 보존**:
       - `isPhotoLoading` 전용 상태를 신설하여 지도 클릭 시점부터 TourAPI 실시간 정품 사진이 준비될 때까지 하드코딩 사진의 섣부른 조기 노출을 100% 원천 차단.
       - 데이터 수신 중에는 인위적 필터 없이 사진 영역 뒤에 실버 스켈레톤 쉬머(`shimmer 1.5s`)만을 은은하게 노출.
