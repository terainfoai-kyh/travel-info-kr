...
- **🪄 [프론트엔드 혁신] URL 딥링크(?city=seoul&days=3&lang=en) 감지 4K 코스 & 확정 타임라인 즉시 직행 런처 전수 구축 (`App.jsx`, `DesktopMapExplorer.jsx`, `MyTripTab.jsx`, `mapTileUtils.js`, `scripts/redditTelegramRadar.js`)**:
  ...
  - **🗺️ 지도 회색 타일(`Map data not yet available`) 결함 100% 척결**: 서울 도심 줌 레벨에서 회색 타일 에러를 내뿜던 Esri 타일을 영구 폐기하고, 한국 전역 줌 19까지 고화질 영문/로마자(`Gyeongbokgung Palace`, `Seoul`)가 선명하게 렌더링되는 글로벌 표준 **CartoDB Voyager** 타일 엔진 탑재.
