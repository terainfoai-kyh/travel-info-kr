...
1. **세션 시작 시 무조건 선(先) 브리핑 의무화**:
   - 사용자가 묻지 않아도 매일 아침 또는 새로운 세션이 시작될 때, 무조건 [`DECISIONS.md`](DECISIONS.md)를 먼저 읽고 **[전날 마지막 작업 내역 + 핵심 설계도/결정사항 + 오늘 바로 이어서 할 작업 1순위]**를 3줄 요약으로 먼저 정갈하게 브리핑하고 업무를 시작한다.
   - 담당 AI가 바뀌거나 세션이 넘어가더라도 사용자가 어제 한 작업을 다시 설명하게 만드는 행위를 100% 원천 금지한다.

2. **100% 전자동 작업 기록 (Zero-Effort Auto-Logging)**:
   - 사용자가 "기억해"라고 말하지 않아도, AI는 주요 기능 완성, 커밋, 배포, 핵심 설계도 합의 시마다 **[날짜, 완성된 기능, 시스템 상태, 다음 할 일]**을 [`DECISIONS.md`](DECISIONS.md)에 100% 자동으로 즉시 갱신 기록한다.
   - 사용자가 "기억해", "기억해둬"라고 명시적으로 지시한 경우, 해당 시점의 설계도나 요구사항을 [★ Golden Checkpoint]로 특급 박제한다.

3. **핵심 아키텍처 영구 계승 및 작업 원복 100% 원천 금지**:
   - 사용자와 합의된 핵심 아키텍처(예: **[보라 AI 자가 학습 플라이휠 5-Lane 파이프라인: 미답변 질문 클라우드 실시간 수집 ➔ 관리자 큐 ➔ 제미나이 배치 학습 ➔ 전 세계 즉시 배포]**)는 세션이 바뀌어도 절대 초기화하거나 원복하지 않고 일관되게 구축 완성한다.
...
11. **Two-Track Cloudflare Pages Deployment Architecture (개발-운영 완벽 분리 & gh-pages 영구 엄금 헌법)**
    - **👑 [선배님 절대 철칙: 운영은 개발이 완료된 후 반영하는 거다]**:
      - **"명심해, 운영은 개발이 완료된 후 반영하는 거다."** (선배님 직강 절대 수칙)
      - 로컬에서 작성된 코드는 반드시 1단계 [개발 서버]에 먼저 배포되어 선배님의 실측 검증(속도, 스팟 정상성, 에러 유무)을 100% 거쳐야 한다.
      - 개발 환경에서 검증되지 않은 코드를 운영 서버에 먼저 밀어 넣거나, 개발을 건너뛰고 운영에 직행하는 행위는 본 프로젝트에서 가장 엄격히 금지되는 **사형급(P0) 헌법 위반**이다.
    - **인프라 원칙**: 본 서비스는 전 세계 300+ 엣지 노드와 10ms 초광속 서빙을 위해 개발과 운영 모두 **Cloudflare Pages 전용 인프라**로 1:1 대칭 완벽 분리 운영된다.
    - **1단계 [개발 서버] (`travelkorea-dev`)**:
      - **서빙 URL**: `https://travelkorea-dev.pages.dev`
      - **배포 명령**: `npm run deploy:dev` (`vite build && npx wrangler pages deploy dist --project-name=travelkorea-dev --commit-dirty=true`)
      - **규칙**: 개발/테스트 단계 배포 지시("배포해", "푸시해") 시 오직 개발 서버로만 배포하며, 운영 사이트는 1픽셀도 건드리지 않고 안전하게 유지한다.
    - **2단계 [운영 서버] (`travel-info-kr` ➔ `koreatravel.cc`)**:
      - **서빙 URL**: `https://koreatravel.cc`
      - **배포 명령**: `powershell.exe -ExecutionPolicy Bypass -File .\scripts\deployProd.ps1` (또는 `npm run deploy:prod`)
      - **규칙**: **오직 개발에서 100% 성능 및 기능 검증 완료 후, 선배님의 명시적 승인("운영 배포해")이 있을 때만** 운영 프로젝트(`travel-info-kr`)로 다이렉트 승격 배포한다.
      - **🚫 단독 git push origin main 실행 100% 영구 엄금**: 소스 푸시만 하고 웹 엣지 반영을 누락하거나 개발 검증 없이 운영에 밀어 넣는 행위를 100% 원천 엄금한다.
    - **🚫 `gh-pages` 브랜치 사용 100% 영구 엄금**:
      - `package.json`이 없는 빌드 브랜치를 푸시하여 Cloudflare 빌드 실패(`ENOENT`) 및 운영 서비스 중단을 유발하는 행위를 영구히 차단한다.
...
19. **Single Source of Truth & Local Ghost Cache Ban (100% 순수 중앙 클라우드 단일 진실 원천 & 로컬 캐시 착시 찌꺼기 영구 금지 헌법)**
    - **관리자 큐 및 커스텀 지식 단일 원천(Single Source of Truth) 통일**: 관리자 화면의 미답변 질문 큐 및 학습된 커스텀 지식은 브라우저 로컬 스토리지(`localStorage`)와 애매하게 섞어두지 않고, **오직 100% Cloudflare 중앙 서버 DB(`/api/qna`)만을 단일 진실 원천으로 직결**하여 바라본다.
...
22. **Strict Ban on Ad-hoc Patching & Mandatory Universal Pipeline (주먹구구식 개별 땜빵 영구 금지 및 단일 표준 파이프라인 헌법)**
    - **개별 도시/명소 if문 땜빵 100% 영구 금지**: 특정 도시(울주, 거창, 김천 등)나 특정 명소(낙안읍성 등)에서 버그나 누락이 발생했을 때, `if (city === '거창')` 식의 단발성 땜빵 조건문이나 특정 도시 전용 하드코딩 딕셔너리를 수동으로 덧붙여 임시 모면하는 행위를 영구히 엄금한다.
    - **단일 표준 파이프라인(Universal Pipeline) 준수 의무**: 모든 수정은 대한민국 226개 모든 시·군·구에 동등하게 적용되는 **단일 표준 알고리즘(자연어 형태소 파싱, 공공 TourAPI 동적 수신, 위경도 공간 클러스터링, 비고정 타임라인)** 레벨에서 근본적으로 해결해야 한다.
    - **지식 유무와 무관한 완전 자립형 코스 완성**: 공식 지식베이스(`CITY_LOCAL_KNOWLEDGE`)에 등록된 대도시뿐만 아니라, 미등록된 전국 226개 어떤 소도시라도 TourAPI 실시간 공공데이터를 기반으로 1일차부터 3일차까지 실제 살아있는 정품 명소 타이틀(예: `거창 수승대`, `김천 직지사`, `신안 퍼플섬`)로 100% 자동 승격 조립되어야 하며, 빈 일차(0개 추천)나 밋밋한 더미 제목이 노출되는 것을 100% 원천 차단한다.
    - **사이드 이펙트 유발 수정 엄금**: 특정 도시나 기능을 수정하면서 기존에 잘 동작하던 다른 화면(화면 탭 전환, 이전 도시 기억 유지 등)을 훼손하거나 꼬이게 만드는 수정을 절대 금지한다.

23. **Strict Ban on Abandoning Broken Source & Zero Session Breakage Rule (에러 소스 방치 및 미완성 세션 이탈 100% 영구 금지 헌법)**
    - **미완성/문법 파손 상태 방치 100% 영구 엄금**: 파일 수정 중 툴 에러나 문법 깨짐(Syntax Corruption)이 발생한 채로 수습하지 않고 세션을 종료하거나 방치하여 선배님께 혼선과 스트레스를 드리는 행위를 100% 원천 금지합니다.
...
25. **Mandatory Local Build & Fast Direct Deployment Rule (로컬 직접 빌드 & 초고속 다이렉트 배포 의무화 헌법)**
    ...
    - **"이제부턴 빌드해서 올리자" 선배님 수칙 영구 계승**:
      ...
      - **개발 배포**: `npm run deploy:dev` (Cloudflare Pages `travelkorea-dev` 다이렉트 엣지 배포)
      - **운영 배포**: `powershell.exe -ExecutionPolicy Bypass -File .\scripts\deployProd.ps1` (검증 ➔ 빌드 ➔ origin/main 푸시 ➔ Cloudflare Pages `travel-info-kr` 다이렉트 엣지 배포 ➔ 해시 검증 완비)
