...
### 🗓️ [2026-08-13] - 3단계: 순수 대화형 Vora AI 컨시어지 & 최악/최선 리스크 파이프라인 합의
- **회의 및 합의 내용**:
  - **순수 대화형 Vora AI 본질 확립**: 단순 DB 목록 렌더링을 100% 지우고, Gemini AI가 유저의 감성("사랑", "힐링", "2박3일")을 읽어 **사람 가이드처럼 1:1 대화체 맞춤 브리핑을 100% 생성하는 순수 대화형 UX** 채택.
  - **Function Calling & 정품 GPS 연동**: Gemini AI 대화 브리핑 하단에 한국관광공사 TourAPI 4.0 정품 도로명 주소 및 위도(`lat`), 경도(`lng`) 지도 핀 결합.
  - **최악 vs 최선 리스크 시뮬레이션 합의**:
    - **최악 (Worst-Case)**: 전환율 0% 시 월간 Max 손실액 = **`-15,000원/월`** (GCP $20 Hard Limit 예산 캡 적용으로 적자 원천 차단)
    - **평균 (Base-Case)**: 유저 1만 명 시 월간 순이익 = **`+1,210,000원/월` (순이익률 99%)**
  - **이름 변경 ➔ 빌드 에러 스캔 리팩토링 적용 (`73a76c3`)**:
    - `extractLocationKeyword` 레거시 정규식을 `_DEPRECATED_`로 변경 ➔ 0 에러 확인 ➔ 레거시 코드 완전 삭제 완료.
- **처리 결과**: `TODO.md` 마스터 전략 및 리스크 파라미터 저장 완료.
