...
## 📱 3. 스마트폰 홈 화면 바로가기 (PWA "홈 화면에 추가") 구축 - [완료]

- [x] **PWA 웹 앱 매니페스트 (`public/manifest.json`) 구축**
  - [x] 앱 이름(Travel Korea / K-Travel Explorer), 독립형 모드(`standalone`), 다크/라이트 테마 색상 설정 완료
  - [x] 192x192 및 512x512 고화질 모바일 홈 화면 전용 'K-태극 3D' 정품 앱 아이콘 에셋 등록 (`apple-touch-icon` 포함)
- [x] **스마트폰 홈 화면 설치 팝업 & 가이드 연동 (`PWAInstallBanner.jsx`)**
  - [x] 안드로이드(Chrome/삼성인터넷): "📲 홈 화면에 바로가기 앱 설치" 팝업 자동 연동
  - [x] 아이폰(iOS Safari): "공유 버튼 ➔ 홈 화면에 추가" 안내 가이드 모달 제공
  - [x] 카카오톡 인앱 브라우저 진입 사용자 대상 카카오 탈출 & 바로가기 추가 안내 배너 연동
